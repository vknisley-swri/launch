...
class ExecuteSshProcess(ExecuteRemoteProcess):
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        Construct an ExecuteSshProcess action.
        ...
        :param: process_description the `launch.descriptions.Executable` to execute
            as a local process
        :param: machine an object defining the SSH connection information,
            such as host, port, user, etc.
    ...
    def execute(self, context: LaunchContext) -> Optional[List[LaunchDescriptionEntity]]:
        ...
        if self.__executed:
            raise RuntimeError(
                f"ExecuteSshProcess action '{name}': executed more than once: {self.describe()}"
            )
